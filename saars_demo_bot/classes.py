...
class My_Pirate(object):
    def __init__(self, game, pirate):
        ...
        if self.target is None:
            ...
            target = self.target.island.id
        game.debug('initial: id: {}; power: {}; target: {}'.format(self.pirate.id, self.power, target))
    ...
    def update_data(self, game):
        ...
        if self.target is None:
            ...
            target = self.target.island.id
        game.debug('updated: id: {}; power: {}; target: {}'.format(self.pirate.id, self.power, target))
    ...
    def set_target(self, game, target):
        if target is None: # Ship is lost - Release island
            if not self.target is None: 
                ...
                original = self.target.island.id
            else:
                original = None
            game.debug('id: {}. Given target is None. Target is changed from: {}'.format(self.pirate.id, original))
            self.target = target
            ...
            if not self.target is None:
                ...
                original = None
            game.debug('id: {}. Given target is {}. Target is changed from: {}'.format(self.pirate.id, target.island.id, original))
            self.target = target
...
class Enemy_Pirate(object):
    ...
    def update_data(self, game):
        game.debug(len(game.all_enemy_pirates()))
        self.pirate = game.get_enemy_pirate(self.pirate.id)
        self.power = self.calculate_power(game)
        
    def calculate_power(self, game):
        power = 0
        if not self.pirate.is_lost and self.is_capturing(game): # If pirate is free
